...
class GuardPlanListView(LoginRequiredMixin, PermissionRequiredMixin, ListView):

    model = GuardPlan
    template_name = 'guard_plan/list.html'
    context_object_name = 'guard_plans'
    paginate_by = 5
    permission_required = 'pwapp.view_guard_plan'

    def get_context_data(self, **kwargs):
        context = super(GuardPlanListView, self).get_context_data(**kwargs)
    ...
    def get_queryset(self):
        object_list = GuardPlan.objects.filter(~Q(students_first_turn=None) | ~Q(students_second_turn=None) )
        return object_list